...
- 正方形资产卡片：固定为 `320 × 320 px`，完整文件名不用滚动并至少保留三行显示空间，底部只保留精简的资产类型条。
- 原生导航交互：空白画布按住左键拖拽平移；资产节点可选中，选中后按住左键拖拽移动，右键可在 Godot FileSystem Dock 中定位文件。
...
点击 **Legend…** 可以随时查看颜色和线型语义。蓝色只表示 Scene 资产，不表示引用方向；方向统一由箭头表达。继承关系固定从子类指向父类，父类祖先层级越高，布局中心权重越大。卡片只显示类型和完整文件名，文件名直接按字符换行且不使用滚动容器；资源路径仍保留在搜索和内部导航数据中。

在画布空白处按住鼠标左键拖拽即可平移视图。单击资产节点会保留 Godot 原生选中状态，选中后按住鼠标左键拖拽可手动调整位置。双击节点打开资产，右键选择 **Show in FileSystem** 可调用 Godot 原生 FileSystem Dock 定位对应文件。
