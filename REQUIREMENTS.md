...
- [ ] \[0.2.0-FE-A-000] 改进大型项目图谱体验 #epic #P1
  ...
  - [ ] \[0.2.0-FE-A-025] 节点固定宽高，正文自动换行并在溢出时支持纵向滚动 #cut（由正方形卡片与精简类型栏取代）
  - [ ] \[0.2.0-FE-A-026] 长标题默认省略，悬停或选中时横向滚动并显示完整标题浮层 #cut（由完整文件名多行显示取代）
  ...
  - [x] \[0.2.0-FE-A-033] 在画布空白处按住鼠标左键拖拽即可平移视图，节点仍按住拖拽移动 #P0
  - [x] \[0.2.0-FE-A-034] 固定宽度节点不显示资源路径，完整文件名按任意字符自动换行 #P0
  - [x] \[0.2.0-FE-A-035] 节点右键菜单使用 Godot 原生 FileSystem Dock 定位对应文件 #P0
  - [x] \[0.2.0-FE-A-036] 每条语义连线绕开除起终点外的全部可见节点卡片，并保留方向与线型 #P0
  - [x] \[0.2.0-FE-A-037] 自动验证完整文件名、右键入口、拖拽平移和节点避障路由
  - [x] \[0.2.0-FE-A-038] 资产节点保持 `320 px` 宽并加高为 `320 × 320 px` 正方形，布局与避障同步使用正方形包围盒 #P0
  - [x] \[0.2.0-FE-A-039] 文件名不使用滚动容器，固定区域至少容纳三行按字符换行文字；资产类型区域缩为底部短条 #P0
  - [x] \[0.2.0-FE-A-040] 资产节点显式可选中，文件名与类型内容不拦截鼠标事件；选中节点后按住左键可拖动 #P0
  - [x] \[0.2.0-FE-A-041] editor smoke 验证正方形尺寸、三行标题空间、无标题滚动容器、节点可选与可拖拽
